...
% 
FilePathStorage=pwd;
% FilePathname1=strcat(FilePathStorage,'/Sad/');
% FileNameListArray=dir([FilePathname1 '*.jpg']);
% FileNameListArray1=dir([FilePathname1 '*.mat']);
% Totalfiles=size(FileNameListArray,1);
% feyeimg=FileNameListArray(1).name;
% eyeimg=[FilePathname1,feyeimg];
% for jjjj=1:Totalfiles
% TempArray=sprintf('Sad/%d.mat',jjjj);
% load(TempArray);
% EEGdata=ictal;
% figure,
% plot(EEGdata);
...
% delete(TempArray)
% save(TempArray,'EEGdata')
% end
TempArray=sprintf('Database/%d.mat',3);
TempArray1=sprintf('Database/%d.jpg',3);
load(TempArray);
...
Xtrain=[Train_dataN;Train_dataH;Train_dataS;Train_dataF]';
Q=[Train_LabN Train_LabH Train_LabS Train_LabF];
TLab=[Train_LabN1;Train_LabH1;Train_LabS1;Train_LabF1];
sizeX=size(Xtrain);
max_iter1=10;
max_iter2=10;
max_iter3=10;
...
X=Xtrain;
h_n1=ceil(size(X,1)/2);
h_n2=ceil(size(X,1)/4);
h_n3=ceil(size(X,1)/8);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
X_test=T_feat';
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Call Function (3-layer deep architecture)
% 1st layer
[w1, w2]=lc_auto_stage_k_n(Xtrain,Q,h_n1,max_iter1,lambda1);
%2nd Layer
 Z=w1*X;
 Z_test=w1*X_test;
[w3, w4]=lc_auto_stage_k_n(Z,Q,h_n2,max_iter2,lambda2);
%3rd Layer
 Z2=w3*w1*Xtrain;
 Z2_test=w3*w1*X_test;
[w5, w6]=lc_auto_stage_k_n(Z2,Q,h_n3,max_iter3,lambda3);
train1=w5*Z2;
test1=w5*Z2_test;
Classfy_Result= knnclassify(test1',train1',TLab,1);
figure,
if(Classfy_Result==1)
    subplot(1,2,1)
    imshow(DataImg)
    title('Eye Tracking Image')
    hold on
    subplot(1,2,2)
    plot(EEGdata)
    title('EEG Signal')
    ha = axes('Position',[0 0 1 1],'Xlim',[0 1],'Ylim',[0 1],'Box','off','Visible','off','Units','normalized', 'clipping' , 'off');
      text(0.5, 1,'\bf Emotion Type: Neutral','HorizontalAlignment','center','VerticalAlignment', 'top');
     pause(0.5)
    msgbox('Emotion Type: Neutral')
elseif(Classfy_Result==2)
   msgbox('Emotion Type: Happy')
elseif(Classfy_Result==3)
   msgbox('Emotion Type: Sad')
elseif(Classfy_Result==4)
   msgbox('Emotion Type: Fear')
end
