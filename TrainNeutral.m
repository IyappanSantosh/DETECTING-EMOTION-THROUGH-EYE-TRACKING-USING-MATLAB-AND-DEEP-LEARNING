...
delete Ndata.mat;
FilePathStorage=pwd;
FilePathname1=strcat(FilePathStorage,'/Neutral/');
FileNameListArray=dir([FilePathname1 '*.jpg']);
FileNameListArray1=dir([FilePathname1 '*.mat']);
Totalfiles=size(FileNameListArray1,1);
Train_dataN=zeros(Totalfiles,18);
Train_LabN=zeros(2,Totalfiles);
Train_LabN1=zeros(Totalfiles,1);
for PPP=1:Totalfiles
TempArray=sprintf('Neutral/%d.mat',PPP);
TempArray1=sprintf('Neutral/%d.jpg',PPP);
load(TempArray);
...
save Ndata.mat Train_dataN Train_LabN Train_LabN1